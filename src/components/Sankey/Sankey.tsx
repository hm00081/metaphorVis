...
export const Sankey = ({
  ...
  const [sourceTargetIdLinksDict, setSourceTargetIdLinksDict] =
    useState<SourceTargetIdLinksDict>({});
  ...
  useEffect(() => {
    ...
    originData.links.forEach((link1) => {
      originData.links.forEach((link2) => {
        if (
          ...
          link1.valueid === link2.valueid &&
          link1.paperName === link2.paperName
        ) {
          const sourceTargetId = `${link2.source}-${link2.target}-${link2.valueid}-${link2.paperName}`;

          if (sourceTargetId in sourceTargetIdLinksDict) {
            const alreadySameLink = sourceTargetIdLinksDict[
              sourceTargetId
            ].find((alreadyLink) => alreadyLink === link2);
            if (!alreadySameLink) {
              //@ts-ignore
    ...
    for (const [sourceTargetId, sameSourceTargetLinks] of Object.entries(
