...
export const Node = ({
  ...
  const onClickFunction = (links: SankeyLinkExtended[]) => {
    ...
    let arrLink = links.filter((recursiveLink, i) => {
      if (
        (node.number === recursiveLink.source ||
          node.number === recursiveLink.target) &&
        recursiveLink.valueid
      ) {
        if (links[i].color !== 'grayLinkColor') {
          const selectedNodePart =
            sourceTargetIdLinksDict[
              `${links[i].source}-${links[i].target}-${links[i].valueid}-${links[i].paperName}`
            ];
    ...
    renderingData.links.forEach((renderingLink) => {
      ...
      arrLink.forEach((linkPart) => {
        if (renderingLink.id && renderingLink.id === linkPart.id) {
          ...
          if (
            (renderingLink.target >= 31 && renderingLink.target <= 33) ||
            (renderingLink.source >= 31 && renderingLink.source <= 33)
          ) {
          ...
          if (
            (renderingLink.target >= 36 && renderingLink.target <= 38) ||
            (renderingLink.source >= 36 && renderingLink.source <= 38)
          ) {
          ...
          if (
            (renderingLink.target >= 47 && renderingLink.target <= 48) ||
            (renderingLink.source >= 47 && renderingLink.source <= 48)
          ) {
...
function findFrontLinks(arg: {
  linkPart: SankeyLink;
  renderingData: SankeyData;
}) {
  ...
  const frontLinks = renderingData.links.filter((renderingLink) => {
    ...
    if (
      renderingLink.target === linkPart.source &&
...
function findBackLinks(arg: {
  ...
  const backLinks = renderingData.links.filter((renderingLink) => {
    ...
    // }
    if (
      renderingLink.source === linkPart.target &&
      renderingLink.process === linkPart.process
    ) {
      if (renderingLink.target >= 31 && renderingLink.target <= 33) {
        renderingLink.color = 'intOneLinkColor';
